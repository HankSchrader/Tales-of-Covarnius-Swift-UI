...
struct Part_4_See_Whats_In_The_Sky: View {
    ...
    var body: some View {
        ...
        let firstChoice = AnyView(Part_1_Dissect_My_Brains().environmentObject(log))
        let text =
        ...
        let secondChoice = AnyView(Part_1_Fork_In_The_Road().environmentObject(log))
        let storyView: StoryPayload = StoryPayload(text: text, firstChoice: firstChoice, decision1: decision1, decision2: decision2, secondChoice: secondChoice)
